@extends('backend.base.index')
@section('title', 'Announcements')
@section('content')
    <div class="card">
        <div class="card-body">
            <div class="card-header d-flex">
                <h5 class="card-title fw-semibold mt-2">Pengumuman</h5>
                <a href="/announcement/create" class="btn btn-primary ms-auto">+ Tambah Data</a>
            </div>
            <div class="card-body">
                <table class="table">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Judul Pengumuman</th>
                            <th>Gambar Pengumuman</th>
                            <th>Tanggal Pengumuman</th>
                            <th>Deskripsi Pengumuman</th>
                            <th>Pengaturan</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($announcements as $item)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $item->announcement_title }}</td>
                                <td><img src="{{ Storage::url($item->announcement_image) }}" alt="" class="img-fluid rounded"
                                        style="width:150px; height:150px; object-fit:contain; object-position:center;"></td>
                                <td>{{ $item->created_at->format('d M Y') }}</td>
                                <td>{{ Str::limit($item->announcement_description, 150, '...') }}</td>
                                <td>
                                    <a href="/announcement/show/{{ $item->id }}"
                                        class="btn btn-outline-info w-100 my-1">Detail</a>
                                    <a href="/announcement/edit/{{ $item->id }}"
                                        class="btn btn-outline-warning w-100 my-1">Edit</a>
                                    <a href="/announcement/destroy/{{ $item->id }}"
                                        onclick="return confirm('Apakah Anda yakin ingin menghapus data ini ?')"
                                        class="btn btn-outline-danger w-100 my-1">Hapus</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </div>
    </div>
@endsection
